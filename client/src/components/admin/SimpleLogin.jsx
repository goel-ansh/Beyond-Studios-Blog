        ...
        try {
          ...
          if(data.success){
            ...
            if (isLogin && endpoint === '/api/user/login') {
              try {
                const adminData = await axios.post('/api/admin/login', {email, password});
                if (adminData.data.success) {
                  setToken(adminData.data.token)
                  setUser(adminData.data.user)
                  localStorage.setItem('token', adminData.data.token)
                  localStorage.setItem('user', JSON.stringify(adminData.data.user))
                  axios.defaults.headers.common['Authorization'] = `Bearer ${adminData.data.token}`;
      ...
      <div className='w-full max-w-md relative z-10'>
        <div className='bg-white/20 backdrop-blur-sm p-8 rounded-3xl shadow-xl border border-blue-100/50'>
          ...
          <form onSubmit={handleSubmit} className='space-y-6'>
              ...
              <button 
                  type="submit" 
                  className='w-full py-4 font-bold text-lg rounded-xl shadow-lg bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white transition-all duration-300 transform hover:scale-105 flex items-center justify-center gap-2'
              >
                  {isLogin ? (
                    <>
